...
use WPSPOTLIGHT\JLTWP_Spotlight;
...
if ( ! class_exists( 'Upgrader' ) ) {
	...
	class Upgrader {
		...
		public function __construct() {
			$this->option_name = JLTWP_Spotlight::plugin_version_key();
		}
		...
		protected function get_installed_version() {
			return get_option( $this->option_name, WPSPOTLIGHT_VER );
		}
		...
		public function if_updates_available() {
			if ( version_compare( $this->get_installed_version(), WPSPOTLIGHT_VER, '<' ) ) {
				return true;
			}
		...
		public function run_updates() {
			...
			$path              = trailingslashit( WPSPOTLIGHT_DIR );
