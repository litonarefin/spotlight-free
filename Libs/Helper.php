...
if ( ! class_exists( 'Helper' ) ) {
	...
	class Helper {
		...
		public static function api_endpoint() {
			...
			$api_endpoint     = apply_filters( 'jltwp_spotlight_endpoint', $api_endpoint_url );
		...
		public static function crm_endpoint() {
			...
			$crm_endpoint     = apply_filters( 'jltwp_spotlight_crm_crm_endpoint', $crm_endpoint_url );
		...
		public static function jltwp_format_string($str) {
			// Replace the parentheses and their contents with -content while removing extra spaces
			$formatted = preg_replace('/\s*\(\s*(.*?)\s*\)\s*/', '-$1', str_replace(' ', '', $str));
			// Return the result without a trailing dash
