...
import { isCSSColorSchemePropSupported, isFirefox } from "../../utils/platform";
...
export function getShadowModifier(value) {
...
// **important**
export function getModifiedFallbackStyle(filter, { strict }) {
...
export function getSelectionColor(theme) {
...
export function getModifiedUserAgentStyle(theme, isIFrame, styleSystemControls) {
    const lines = [];
    if (!isIFrame) {
        lines.push("html {");
        lines.push(
...
export function getBgImageModifier(value, rule, ignoreImageSelectors, isCancelled) {
    try {
        ...
        const getGradientModifier = (gradient) => {
            ...
            return (filter) => {
                return `${typeGradient}(${parts.map((modify) => modify(filter)).join(", ")})${
                    hasComma ? ", " : ""
                }`;
            };
        };
...
export function getShadowModifierWithInfo(value) {
    try {
        let index = 0;
