...
export function replaceCSSVariablesNames(value, nameReplacer, fallbackReplacer) {
...
function wrapBgColorVariableName(name) {
  return `--spotlight-bg${name}`;
}

function wrapTextColorVariableName(name) {
  return `--spotlight-text${name}`;
}

function wrapBorderColorVariableName(name) {
  return `--spotlight-border${name}`;
}

function wrapBgImgVariableName(name) {
  return `--spotlight-bgimg${name}`;
}
...
function isTextColorProperty(property) {
  return (
    property === "color" || property === "caret-color" || property === "-webkit-text-fill-color"
  );
}
