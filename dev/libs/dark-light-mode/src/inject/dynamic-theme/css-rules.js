...
export function iterateCSSRules(rules, iterate, onMediaRuleError) {
    forEach(rules, (rule) => {
...
export function iterateCSSDeclarations(style, iterate) {
...
export function getCSSURLValue(cssURL) {
    return cssURL
        .trim()
        .replace(/[\n\r\\]+/g, "")
        .replace(/^url\((.*)\)$/, "$1")
...
export function getCSSBaseBath(url) {
...
export function replaceCSSRelativeURLsWithAbsolute($css, cssBasePath) {
...
export function replaceCSSFontFace($css) {
