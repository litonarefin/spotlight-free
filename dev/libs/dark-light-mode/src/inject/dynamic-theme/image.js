...
export async function getImageDetails(url) {
...
export function getFilteredImageDataURL({ dataURL, width, height }, theme) {
  const matrix = getSVGFilterMatrixValue(theme);
  const svg = [
    `<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="${width}" height="${height}">`,
    "<defs>",
    '<filter id="spotlight-image-filter">',
    `<feColorMatrix type="matrix" values="${matrix}" />`,
    "</filter>",
    "</defs>",
    `<image width="${width}" height="${height}" filter="url(#spotlight-image-filter)" xlink:href="${dataURL}" />`,
    "</svg>",
  ].join("");
