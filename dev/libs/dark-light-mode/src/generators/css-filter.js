...
export function hasPatchForChromiumIssue501582() {
    return (
        __CHROMIUM_MV3__ ||
        Boolean(__CHROMIUM_MV2__ && compareChromeVersions(chromiumVersion, "81.0.4035.0") >= 0)
    );
}
...
export function hasFirefoxNewRootBehavior() {
    return Boolean(isFirefox && compareChromeVersions(firefoxVersion, "102.0") >= 0);
}
...
export function getCSSFilterValue(config) {
    const filters = [];
 ...
 */
export function getInversionFixesFor(url, fixes, index) {
